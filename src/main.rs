use srvcs_samplestddev::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-samplestddev listening");
    let deps = Deps {
        samplevariance_url: cfg.samplevariance_url.clone(),
